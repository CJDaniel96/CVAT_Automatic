...
# config.read('config_CHIP_RC.ini')

average_predict_time = 0
average_cls_1_time = 0
average_cls_2_time = 0
...
class OD_models:
    _defaults = {'label_cls' : ['NG', 'OK'],
                 'model_od_para' : {},
                 'model_cls_para' : {},
                 'model_cls_path' : '',
                 'cfg' : './models/JQ_SMT_B_CHIPRC/config_CHIP_RC.ini'}
    @classmethod
    ...
    def model_predict(self, img_path, model_od, imgsz, save_dir , OUTPUT_IMG = '1' ):
        ...
        if OUTPUT_IMG == '1':
            ...
            TEMP_PATH = save_image_folder_path
            ...
            for idx, location in enumerate(defect_position_xml):
                save_path = save_image_folder_path + '\\' + str(defect_name_xml[idx])
...
if __name__ == "__main__":
    ...
    od_model_path     = r'D:\DanielTsai\Code\AIPlatform\CVAT\yolo_inference\models\JQ_SMT_B_CHIPRC\od_model_20220714.pt'
    ...
    image_folder_path = r'D:\DanielTsai\Code\AIPlatform\CVAT\cls\JQ_SMT_B_CHIPRC\datasets\CHIP_RC_T3_20220715112137\T3\train'
    save_image_folder_path = r'D:\DanielTsai\Code\AIPlatform\CVAT\yolo_inference\output'
  
    image_type = '*.jpg'
    check_image_path = image_folder_path + '/' + image_type
    print(check_image_path)
    image_available = glob.glob(check_image_path)
    

    #start_increment_time = time.time()
    save_dir_main = increment_path(Path(project_global) / name_global , exist_ok=False)  # increment run
    #end_increment_time = time.time()
    #print("increment time : " , end_increment_time - start_increment_time)
    
    #global average_predict_time,average_cls_1_time,average_cls_2_time,average_yolov5_time
    index = 0
    do_image, do_foler = parser.get_file(image_folder_path)
    for path_name, foldername in zip(do_image, do_foler):
